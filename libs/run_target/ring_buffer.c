/*!
  ...
  $Id: ring_buffer.c 1927 2010-09-26 23:09:26Z satofumi $
*/
...
int ring_write(ring_buffer_t *ring, const char *data, int size)
{
    ...
    if (ring->first <= ring->last) {
        ...
        byte_move(&ring->buffer[ring->last], data, push_size);
        ring->last += push_size;
    }
    return push_size;
}
...
{
    // データ取得
    int current_size = ring_size(ring);
    int pop_size = (size > current_size) ? current_size : size;
    ...
    if (ring->first <= ring->last) {
        ...
        int move_size = (pop_size < to_end) ? pop_size : to_end;
